...
function showGameScoreLayer() {
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = 'jc鸽鸽看到过的最强者：&nbsp;&nbsp;' + bast;
    l.style.display = 'block';
}
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '你还是乖乖在家做白日梦吧！';
    if (score <= 99) return '有jc幼儿园时的三成功力了~';
    if (score <= 119) return 'jc会对您说一句wtf?';
    if (score <= 129) return 'jc或许应该向您学习！';
    if (score <= 139) return 'jc好像成为您这样的人呐！';
    if (score <= 149) return '您的手速足以为jc买一台ps5了！';
    if (score <= 159) return '您可取jc而代之！';
    return '非人哉!';
}
